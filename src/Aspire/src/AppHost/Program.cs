...
var identityApi = builder
    ...
    .WaitFor(rabbitmq)
    .WithHttpEndpoint(port: 5101, name: "identity-http")
    .WithHttpsEndpoint(port: 5001, name: "identity-https");
...
var flightApi = builder
    ...
    .WithReference(rabbitmq)
    .WaitFor(rabbitmq)
    .WithHttpEndpoint(port: 5102, name: "flight-http")
...
var gateway = builder
    .AddProject<Gateway_Api>("gateway")
    .WaitFor(identityApi)
    .WaitFor(flightApi)
    .WaitFor(passengerApi)
    .WaitFor(bookingApi)
    .WithHttpEndpoint(port: 3001, name: "gateway-http")
    .WithHttpsEndpoint(port: 3000, name: "gateway-https");
