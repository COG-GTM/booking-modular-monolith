...
// Microservice hosts (one per bounded context) plus the API gateway.
// These run side by side with the monolith "api" project during the migration.
...
var bookingApi = builder
    ...
    .WithReference(rabbitmq)
    .WaitFor(rabbitmq)
    .WithEnvironment("Grpc__FlightAddress", flightApi.GetEndpoint("https"))
    .WithEnvironment("Grpc__PassengerAddress", passengerApi.GetEndpoint("https"))
    .WaitFor(flightApi)
...
builder
    ...
    .WithEnvironment(
        "ReverseProxy__Clusters__identity__Destinations__default__Address",
        identityApi.GetEndpoint("https")
    )
    .WithEnvironment("ReverseProxy__Clusters__flight__Destinations__default__Address", flightApi.GetEndpoint("https"))
    .WithEnvironment(
        "ReverseProxy__Clusters__passenger__Destinations__default__Address",
        passengerApi.GetEndpoint("https")
    )
    .WithEnvironment("ReverseProxy__Clusters__booking__Destinations__default__Address", bookingApi.GetEndpoint("https"))
    .WaitFor(identityApi)
    .WaitFor(flightApi)
    .WaitFor(passengerApi)
    .WaitFor(bookingApi);
